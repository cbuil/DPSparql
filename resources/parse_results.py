...
def parse_results(input_filename, output_filename):
    with open(input_filename) as f:
        with open(output_filename, 'w') as fw:
            fw.write(
                "query\tactualResult\tpercentage_error\tgraphSize\tmaxK\tscale\tepsilon\tSensitivity\telasticStability\taveragePrivateResult\tstandardDeviation\n")
            for line in f.readlines():
                ...
                fw.write(str(abs(np.average(private_result))) + str('\t'))
                fw.write(str(abs(np.std(private_result))) + str('\t'))
